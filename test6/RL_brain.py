...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [n_l1, self.output], initializer=w_initializer, collections=c_names)
                b2 = tf.get_variable('b2', [1, self.output], initializer=b_initializer, collections=c_names)
    ...
    def choose_action(self, observation):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            B = np.transpose(actions_value)
            Y = util.lin2(B,V)
            a = util.stochasticAccept(X)
            o = util.stochasticAccept(Y)
